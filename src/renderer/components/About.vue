<template>
  <div @click="handlerAboutClick" v-if="isShow" class="about-box">
    <div @click.stop class="box">
      ...
      <a-row v-for="(item,index) in info" :key="index" :class="{'mt20':index!=0}" :gutter="20">
        <a-col :span="span1">
          <div>{{item.key}}</div>
        </a-col>
        <a-col :span="span2">
...
export default {
  ...
  computed: {
    info() {
      return [
        { key: `${this.$t("common.author")}: `, val: "penguinway" },
        {
          key: `${this.$t("common.technology")}: `,
          val: "electron, vue, node, element-ui"
        },
        {
          key: `${this.$t("common.address")}: `,
          val: "https://github.com/penguinway"
        },
